...
- `GET /fixtures/:fixtureId/context` — same `requireAnyPermission("match.report", "results.verify")` gate and dual-audience resolution as the events `GET` above (`resolveFixtureForReporterOrVerifier()` in `fixture.service.ts`, extracted out of `match-event.service.ts` once this endpoint needed the identical logic). Returns `{ fixtureContext: { fixture_id, home_team: {id, name}, away_team: {id, name} } }` — resolved by walking `home_entry_id`/`away_entry_id` → `CompetitionEntry` → `Team`. Exists because a plain `match.report` holder otherwise has **no** authorized way to learn even the two teams' names: `Team` reads are `roster.manage` gated and `CompetitionEntry` reads are `competition.manage` gated, neither of which a Reporter holds. Deliberately minimal (no venue, no player rosters) — reporter-app's event-logging form only requires picking a team, nothing else.
...
**Standings are input, not computed** — even though `standings.service.ts`'s live table computation (see Standings above) exists now. `entries` is the final order of `CompetitionEntry` ids for that tier's competition, best to worst, supplied directly by the caller, rather than derived from `computeCompetitionTable()`. That's deliberate, not a leftover from before standings computation existed: ending a season is a one-off admin action confirming a specific, final outcome, independent of whatever the live computed table happens to say at the moment someone calls this endpoint (e.g. a late correction to an event after the "final" matchday, or the admin's own judgment call on a dispute) — see `league-system.service.ts`'s `endSeason` for the full reasoning. `CompetitionStanding` (an effectively immutable model — no update/delete route, fields `immutable: true`) locks whatever order is submitted. A `(competition_id, season)` unique index stops the same tier/season from being locked twice (409).

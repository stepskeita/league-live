...
LeagueLive's on-the-ground reporter app (React Native/Expo, file-based routing via `expo-router`). Lets a field reporter sign in, see the fixtures assigned to them (FR24), run a match session (start, log goal/card/substitution/half time/full time, end — FR25-FR27), and hand off to post-match confirmation (FR28).
...
- `components/` — `Button`, `Screen`, `TextField`, `ErrorBanner`, `FixtureCard` (auth/home) plus `ActionTile`, `SegmentedToggle`, `MinuteStepper`, `EventLogModal`, `EventListItem` (match session) — the whole visual language in one place
- `lib/` — `api.ts` (the shared API client, configured with this app's token storage), `auth-context.tsx` (auth state + effective permissions), `fixtures-context.tsx` (the fetched-once "my fixtures" list every screen reads from), `match-session.ts` (pure helpers: score computation, elapsed-minute default, client event id generation), `token-store.ts` (`expo-secure-store`-backed token persistence)
...
**Idempotency (FR27).** A `client_event_id` is generated once per form open (`generateClientEventId()` — a timestamp plus a short random suffix, not a UUID; nothing here needs cryptographic randomness, just "unique enough for one reporter's device within one match", so no new dependency for it) and stays fixed across retries within that same open: if a submission fails, the modal stays open with an error and "try again" reuses the *same* id, matching what `POST /fixtures/:id/events` is idempotent on. What this app does **not** build: a persistent offline queue that survives the app closing and auto-syncs on reconnect — that's a materially bigger feature (background sync, conflict handling) than "screens with large tap controls and minimal required fields" asked for, and is a reasonable next step on top of this.
